...
grep -i root /etc/ssh/sshd_config   # -i 不区分大小写
grep "Oct\ 26\ 17:52" /var/log/secure  # 过滤带空格的字符串“Oct 26 17:52”，空格需要'\'转义
grep '^[^#].*$' /etc/ssh/sshd_config  # 输出不以#开头的行
```
...
| **p**    | 打印当前模式空间内容，追加到默认输出之后。sed -n "2p" a.txt # 打印第2行                     |
| **a**    | 在指定行**后面追加**文本，支持使用\n实现多行追加。sed "2a123" demo \#在第2行后加123。sed -i '/^GRUB_CMDLINE_LINUX/ s/$/\"/' /etc/default/grub，追加引号。sed -i '$athis is b\n' a.txt # 用$表示文件尾时，须用单引号，双引号中的$用来取变量值 |
